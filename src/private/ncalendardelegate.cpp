...
NCalendarDelegate::NCalendarDelegate(NCalendarModel* calendarModel, QObject* parent)
    : QStyledItemDelegate{parent}
{
    ...
    connect(_calendarModel, &NCalendarModel::displayModeChanged, this, &NCalendarDelegate::onCalendarModelDisplayModeChanged);
}
...
void NCalendarDelegate::_drawYearOrMonth(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
        ...
        (displayMode == NCalendarType::MonthMode && data.month == _nowDate.month() && data.year == _nowDate.year()))
    {
        ...
        QColor drawColor = option.state.testFlag(QStyle::State_MouseOver) ? 
            accentColor.lighter(110) : accentColor;
    ...
    if (isNow) {
        textColor = _isDark ? 
            NThemeColor(NFluentColorKey::TextOnAccentFillColorPrimary, NThemeType::Dark) :
    ...
    painter->drawText(itemRect, Qt::AlignCenter, displayMode == NCalendarType::YearMode ? 
                    QString::number(data.year) : QString::number(data.month));
...
void NCalendarDelegate::_drawDays(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    qreal penWidth = 1.5;
    ...
    if (variant.isValid())
    {
        ...
        else
        {
            ...
            {
                QColor accentColor = nTheme->accentColor().normal();
                QColor borderColor = option.state.testFlag(QStyle::State_MouseOver) ? 
                    accentColor.lighter(110) : accentColor;
